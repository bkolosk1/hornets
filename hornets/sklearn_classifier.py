...
class HorNetClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _augment_features_with_synthetic(self, features: np.ndarray) -> np.ndarray:
        ...
        if isinstance(features, np.matrix):
            ...
            try:
                joint_space[:features.shape[0], :features.shape[1]] = features[:, :].todense()
    ...
    def fit(
        ...
        if self.feature_names is None:
            ...
            self.feature_names_ = self.feature_names + [f"synth{x}" for x in range(self.order)]
        ...
        for epoch in tqdm(range(self.num_epochs), desc="Training Epochs", disable=not self.verbose):
    ...
    def get_top_rules(self, top_k: int = 3) -> None:
        ...
        for idx in top_indices:
            ...
            feature_combination = [
                feature_names[i] for i in combination 
                if "synth" not in feature_names[i]
            ]
            score = combination_scores[idx]
            logging.info(f"Feature Combination: {feature_combination}, Score: {score:.4f}")
