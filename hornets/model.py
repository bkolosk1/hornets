...
class HorNet(nn.Module):
    def __init__(self,
        ...
        self.comb_space = torch.nn.Parameter(data=torch.randn(
            order, len(self.comb_indices)),
                                             requires_grad=True)
    ...
    def get_rules(self):
        ...
        for j in sindices:
            features = [
                str(self.feature_names[x]) for x in list(self.comb_indices[j])
            ]
