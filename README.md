...
|-------------------|-------------------------|------------:|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------|
| `batch_size`      | `int`                  | 4           | Number of samples per training batch. Increasing `batch_size` can speed up training but may require more memory and can affect convergence dynamics.                                                                                                                                            | 4, 8, 16, 32                         |
| `num_epochs`      | `int`                  | 1000        | Maximum number of epochs (complete passes through the training data). Large values allow more training time but risk overfitting. Early stopping (`stopping_crit`) helps mitigate overfitting.                                                                                                 | 100, 300, 500, 1000                  |
...
| `stopping_crit`   | `int`                  | 10          | Number of consecutive epochs without improvement on validation metric to wait before stopping training early. Prevents unnecessary training once model performance plateaus.                                                                                                                     | 5, 10, 15                            |
| `feature_names`   | `Optional[List[str]]`  | None        | Names of input features. Useful for interpretability and debugging, but not typically a hyperparameter to tune.                                                                                                                                                                                   | Not applicable                       |
...
| `activation`      | `str`                  | "polyclip"  | Activation function used in the model. Options include `"polyclip"` or other functions. The choice can have a significant impact on model performance and training stability, but is more of a structural decision than a numeric hyperparameter.                                                 | "polyclip", "relu", "sigmoid", etc.  |
